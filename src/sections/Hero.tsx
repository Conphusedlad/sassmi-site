...
const TINS = [
  { slug: 'pudina', cls: 'left-[2%] bottom-[6%] h-[62%] z-10', delay: 0.35, float: '8s' },
  { slug: 'jaggery', cls: 'left-1/2 -translate-x-1/2 bottom-[10%] h-[78%] z-20', delay: 0.15, float: '9s' },
  { slug: 'dry-fruits-kheer', cls: 'right-[2%] bottom-[4%] h-[64%] z-10', delay: 0.5, float: '7.5s' },
]

export function Hero() {
  return (
    <section id="top" className="relative overflow-hidden bg-night text-ivory grain">
      ...
      <div className="relative mx-auto grid min-h-[100svh] max-w-7xl items-center gap-10 px-5 pb-16 pt-28 sm:px-8 lg:grid-cols-12 lg:pt-24">
        <div className="relative z-10 lg:col-span-6">
          <motion.p initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.9, ease: EASE, delay: 0.1 }} className="kicker">Premium makhana · Rooted in Mithila</motion.p>
          ...
          <motion.p initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 1, ease: EASE, delay: 0.38 }} className="mt-7 max-w-lg text-[17px] leading-relaxed text-ivory/72 sm:text-lg">
            A timeless superfood from the ponds of Mithila, roasted in olive oil and seasoned with honest ingredients — nine flavours, one midnight-blue tin.
          </motion.p>
        ...
        <div className="relative h-[440px] sm:h-[520px] lg:col-span-6 lg:h-[660px]">
          <div className="absolute left-1/2 top-[70%] h-40 w-[80%] -translate-x-1/2 rounded-[100%] bg-gold/20 blur-3xl" />
          {TINS.map((t) => (
            <motion.div key={t.slug} initial={{ opacity: 0, y: 70 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 1.3, ease: EASE, delay: t.delay }} className={`absolute ${t.cls}`}>
              <img src={asset(`/img/tins-cut/${t.slug}.png`)} alt="" className="animate-float h-full w-auto drop-shadow-[0_50px_60px_rgba(0,0,0,.55)]" style={{ animationDuration: t.float, animationDelay: `${t.delay}s` }} fetchPriority="high" />
            </motion.div>
          ))}
        </div>
      </div>
      <motion.a href="#collection" initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 1.6, duration: 1 }} className="absolute bottom-6 left-1/2 hidden -translate-x-1/2 flex-col items-center gap-2 text-[10px] uppercase tracking-[.3em] text-ivory/50 hover:text-gold md:flex" aria-label="Scroll to the collection">
