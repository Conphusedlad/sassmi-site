...
import { cart, uiStore, useUI } from '../lib/cart'
      ...
      {p && (
        <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="fixed inset-0 z-[75] flex items-end justify-center bg-night/70 p-0 backdrop-blur-sm sm:items-center sm:p-6" onClick={() => uiStore.quickView(null)}>
          <motion.div initial={{ y: 40, opacity: 0, scale: 0.98 }} animate={{ y: 0, opacity: 1, scale: 1 }} exit={{ y: 20, opacity: 0 }} transition={{ duration: 0.5, ease: EASE }} onClick={(e) => e.stopPropagation()} role="dialog" aria-label={`${p.name} quick view`}
            ...
            <div className="relative flex items-center justify-center p-8" style={{ background: `radial-gradient(80% 60% at 50% 100%, ${p.hex}33 0%, transparent 70%), linear-gradient(180deg,#FAF6EE,#EFE7D6)` }}>
              <img src={tinCut(p)} alt={p.name} className="h-[380px] w-auto drop-shadow-[0_40px_40px_rgba(15,26,48,.3)] sm:h-[460px]" />
              <span className="kicker absolute left-6 top-6">{p.kind === 'bundle' ? 'Gift box' : `${p.profile} · ${p.netWeight}`}</span>
            </div>
            <div className="relative flex flex-col p-7 sm:p-9">
              ...
              <h3 className="text-[40px] leading-none">{p.name}</h3>
              ...
              <p className="mt-3 text-xs text-muted">{p.ingredientHint}</p>
              <div className="mt-auto flex flex-wrap items-center gap-4 border-t border-ivory-2 pt-6">
                <p className="font-display text-3xl">{formatINR(p.price)}</p>
                <div className="flex items-center rounded-full border border-ink/15">
                  <button className="flex h-10 w-10 items-center justify-center" onClick={() => setQty((q) => Math.max(1, q - 1))} aria-label="Decrease"><Minus size={14} /></button>
                  <span className="w-8 text-center">{qty}</span>
                  <button className="flex h-10 w-10 items-center justify-center" onClick={() => setQty((q) => Math.min(12, q + 1))} aria-label="Increase"><Plus size={14} /></button>
                </div>
                <button className="btn btn-night flex-1" onClick={() => { cart.add(p.slug, qty); toast(`${p.name} × ${qty} added`); uiStore.quickView(null); uiStore.openCart() }}>Add to cart</button>
              </div>
              <Link to={`/product/${p.slug}`} onClick={() => uiStore.quickView(null)} className="mt-4 text-center text-xs uppercase tracking-[.2em] text-ink-soft underline-offset-4 hover:text-gold-deep hover:underline">Full details →</Link>
            </div>
          </motion.div>
        </motion.div>
      )}
