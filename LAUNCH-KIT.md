...
**Coming soon mode — 19 Sep 2026 (live site):** on Shivansh's instruction the Sassmi range is "coming soon" and every buying path goes to WhatsApp (+91 98688 15333). One switch controls it: `business.store.open` in `shared/config.ts` (currently **false**). While false: no Sassmi prices or pack sizes anywhere (cards, quick view, product pages, flavour stage, gifting, footer), every add-to-cart is "Ask on WhatsApp", the cart is hidden, `/checkout` and `/order/…` show "Online ordering opens soon", the where-to-buy card reads "Online store · Coming soon", FAQ answers say ordering hasn't opened, the terms/shipping/refund policies carry a "from launch" note and hide the placeholder fees, and the server refuses `/checkout/order`. Crunchy Makhana is untouched. On the static GitHub Pages build (sassmi.com) the contact form, launch list and reviews also go to WhatsApp, because there is no server behind it. **To open the store:** (1) confirm prices, pack size and FSSAI number; (2) deploy the full server version (Vercel + Supabase + Razorpay keys, steps below) — never flip the switch on the GitHub Pages build, which only has a simulated checkout; (3) set `store: { open: true }`; (4) change "Launching soon." in the `index.html` meta description back to an ordering line.
